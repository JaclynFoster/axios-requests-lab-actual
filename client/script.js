...
// CODE HERE
const sayHelloBackground = () => {
    sayHelloButton.style.backgroundColor = "black"
    sayHelloButton.style.color = "white"
}

sayHelloButton.addEventListener("mouseover", sayHelloBackground)
// PROBLEM 3
/*
...
sayHelloButton.addEventListener("mouseout", buttonMouseout)
// PROBLEM 4
/*
...
// PROBLEM 6 
/*
    Now lets see if you can send a request param! inside repeatMyParam function below  make get
     request to 'http://localhost:3000/repeat/{SOMEPARAM}', but with a string instead of {SOMEPARAM}.  
...
/*
    ...
    Inside the repeatMyParam function above, grab the element with the 
    id of 'repeat-text' and set its textContent property equal to the response data.
...
const queryText = () => {
    axios.get(`http://localhost:3000/query-test?name=Jaclynage=34`)
    .then((response) => {
        console.log(response.data)
    })
    .catch((error) => {
...
queryButton.addEventListener("click", queryText)
////////////////
//INTERMEDIATE//
////////////////
...
/*
    ...
    //It says I sent an empty query
    2: Send more than 1 query on the URL -- what happened? 
*///It concatenated both queries together into 1 query
...
//PROBLEM 11
/*
    You are going to add the ability to POST to the server. You'll need to create a small 
    form and write a function that makes a post request. Then you'll attach that function 
    to the submit event on the form. We'll be creating a list of foods. 

    In the index.html file inside of the client folder, create a form with one text input
     field and a button. The input field should have a placeholder that tells the user to enter
      a food. And the button should indicate that it will add food into a list. 

    In this file (script.js), create a function called createFood. 
    
    Inside the function, select the input you just created in the HTML and save it to a variable
     called foodInput. 
    
    Next, create an object called body inside the function. It should have one key-value pair. 
    The key should be newFood (make sure to match the case and spelling exactly) and the value 
    ...
    Use a .then to handle the promise returned from the axios call. Pass a callback function to the 
    .then. Inside that callback, console log the res.data. 

    Based on what we did earlier to display this type of data, write code that will display the 
    response in your HTML document. 
*/
